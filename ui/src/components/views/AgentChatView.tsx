...
const AgentChatView: React.FC = () => {
  ...
  const upsertStreamingMessage = (messageId: string, delta: string) => {
    setMessages((prevMessages) => {
      const index = prevMessages.findIndex((item) => item.id === messageId);
      if (index === -1) {
        return [
          ...prevMessages,
          {
      ...
      nextMessages[index] = {
        ...nextMessages[index],
        content: `${nextMessages[index].content || ""}${delta}`,
      };
      return nextMessages;
    });
  };

  const finalizeStreamingMessage = (message: ChatMessageVO | undefined) => {
    if (!message) return;
    setMessages((prevMessages) => {
      const index = prevMessages.findIndex((item) => item.id === message.id);
      if (index === -1) {
  ...
  useEffect(() => {
    ...
    const es = new EventSource(`${apiOrigin}/sse/connect/${chatSessionId}`);
    es.onmessage = (event) => {
    ...
    es.addEventListener("message", (event) => {
      ...
      } else if (message.type === "AI_CONTENT_START") {
        const messageId =
          message.metadata?.chatMessageId || `stream-${chatSessionId}`;
        upsertStreamingMessage(messageId, "");
      } else if (message.type === "AI_CONTENT_DELTA") {
        const messageId =
          message.metadata?.chatMessageId || `stream-${chatSessionId}`;
        upsertStreamingMessage(messageId, message.payload.contentDelta || "");
      } else if (message.type === "AI_CONTENT_DONE") {
        finalizeStreamingMessage(message.payload.message);
      } else if (message.type === "AI_CONTENT_ERROR") {
        const errorText = message.payload.statusText || "生成失败";
        setDisplayAgentStatus(true);
        setAgentStatusText(errorText);
        setAgentStatusType("AI_CONTENT_ERROR");
