...
import type { ChatMessageVO, Citation, SseMessageType, ToolCall, ToolResponse } from "../../../types";
...
const ToolResponseDisplay: React.FC<{ toolResponse: ToolResponse }> = ({
  ...
  try {
    ...
    if (
      ...
      const context = String((parsedData as { context?: string }).context || "");
      dataPreview = context.length > 100 ? context.slice(0, 100) + "..." : context;
    } else {
          ...
          <div className="text-xs text-gray-600 font-mono">
            {isJson ? (
              <pre className="whitespace-pre-wrap break-words overflow-x-auto max-h-60 overflow-y-auto">
                ...
                parsedData !== null &&
                "context" in parsedData
                  ? String((parsedData as { context?: string }).context || "")
...
const CitationDisplay: React.FC<{ citations: Citation[] }> = ({ citations }) => {
  const [expanded, setExpanded] = useState(false);
          ...
          {citations.map((citation) => (
            ...
            >
              <div className="flex items-center gap-2 text-gray-700">
                <span className="font-mono text-blue-600">[{citation.index}]</span>
                <span className="font-medium truncate">
                  {citation.docName || "未知文档"}
                </span>
              ...
              {citation.title && (
                <div className="mt-1 text-gray-500">{citation.title}</div>
              )}
              {citation.snippet && (
