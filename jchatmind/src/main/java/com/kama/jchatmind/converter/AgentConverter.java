...
public class AgentConverter {
    ...
    private void normalizeAgentDTO(AgentDTO dto) {
        if (dto.getAllowedTools() == null) {
            dto.setAllowedTools(new ArrayList<>());
        }
        if (dto.getAllowedKbs() == null) {
            dto.setAllowedKbs(new ArrayList<>());
        }
        dto.setAllowedTools(new ArrayList<>(dto.getAllowedTools()));
        dto.setAllowedKbs(new ArrayList<>(dto.getAllowedKbs()));

        AgentDTO.ChatOptions defaults = AgentDTO.ChatOptions.defaultOptions();
        AgentDTO.ChatOptions options = dto.getChatOptions();
        if (options == null) {
            dto.setChatOptions(defaults);
            return;
        }
        if (options.getTemperature() == null) {
            options.setTemperature(defaults.getTemperature());
        }
        if (options.getTopP() == null) {
            options.setTopP(defaults.getTopP());
        }
        if (options.getMessageLength() == null) {
            options.setMessageLength(defaults.getMessageLength());
        }
        if (options.getExecutionMode() == null) {
            options.setExecutionMode(defaults.getExecutionMode());
        }
    }
}
