...
import com.kama.jchatmind.agent.tools.RegistryToolCallback;
...
public class JChatMindFactory {
    ...
    private final ExpertAgentRouter expertAgentRouter;
    ...
    private List<ToolCallback> buildToolCallbacks(List<Tool> runtimeTools) {
        Set<String> runtimeToolSources = runtimeTools.stream()
                .map(Tool::getName)
                .collect(Collectors.toSet());
        return toolRegistry.listDefinitions().stream()
                .filter(definition -> runtimeToolSources.contains(definition.getSource()))
                .map(RegistryToolCallback::new)
    ...
    /**
     * 创建一个 JChatMind 实例，并把长期记忆召回结果注入本轮上下文。
     */
    public JChatMind create(String agentId, String chatSessionId, String userInput) {
        Agent agent = loadAgent(agentId);
        AgentDTO agentConfig = toAgentConfig(agent);
        List<Message> memory = conversationContextService.loadContextMessages(
                chatSessionId,
                agentConfig.getChatOptions().getMessageLength()
        );
        String memoryPrompt = "";
        try {
            memoryPrompt = memoryService.buildMemoryPrompt(agentId, userInput, 8);
        } catch (Exception e) {
            log.warn("长期记忆召回失败，本轮降级为会话上下文: {}", e.getMessage());
        }
        if (StringUtils.hasLength(memoryPrompt)) {
            memory.add(0, new SystemMessage(memoryPrompt));
        }
