...
public class RagMcpServerController {
    ...
    private Map<String, Object> initialize() {
        return Map.of(
                ...
                "serverInfo", Map.of(
                        "name", "mindpilot-rag-memory-mcp-server",
                        "version", "0.1.0"
                )
        );
    }
    ...
    private List<Map<String, Object>> tools() {
        return List.of(
                tool("rag.hybrid_search", "执行 MindPilot RAG 混合检索，返回向量召回、BM25 粗排、rerank 精排后的片段。",
                        Map.of("kbId", "string", "query", "string"),
                        List.of("kbId", "query"),
                        Map.of("capability", "rag", "readOnlyHint", true)),
                tool("memory.save_long_term", "写入 Mem0 风格长期记忆，并生成向量用于后续召回。",
                        Map.of("agentId", "string", "sessionId", "string", "content", "string"),
                        List.of("agentId", "sessionId", "content"),
                        Map.of("capability", "memory", "readOnlyHint", false, "memoryLayer", "long_term")),
                tool("memory.recall", "按语义相似度、重要性和时间衰减召回 Agent 记忆。",
                        Map.of("agentId", "string", "query", "string", "limit", "integer"),
                        List.of("agentId", "query"),
                        Map.of("capability", "memory", "readOnlyHint", true))
        );
    }
    ...
    private Map<String, Object> tool(
        ...
        properties.forEach((key, type) -> jsonProperties.put(key, propertySchema(key, type)));
        return Map.of(
                "name", name,
    ...
    private Map<String, Object> propertySchema(String key, String type) {
        Map<String, Object> schema = new LinkedHashMap<>();
        schema.put("type", type);
        schema.put("description", switch (key) {
            case "kbId" -> "知识库 ID。";
            case "query" -> "用户问题或检索查询。";
            case "agentId" -> "Agent ID。";
            case "sessionId" -> "会话 ID。";
            case "content" -> "需要写入长期记忆的稳定事实、偏好或目标。";
            case "limit" -> "召回记忆数量，默认 5。";
            default -> key;
        });
        if ("limit".equals(key)) {
            schema.put("minimum", 1);
            schema.put("maximum", 20);
        }
        return schema;
    }
